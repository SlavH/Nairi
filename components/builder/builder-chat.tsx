...
interface BuilderChatProps {
  ...
  /** Currently selected free OpenCode model id (e.g. opencode/big-pickle) */
  currentModel?: string
  /** Called when the user picks a different model in the footer dropdown */
  onModelChange?: (id: string) => void
  /** True while the OpenCode WebContainer backend is booting */
  isBooting?: boolean
}
...
export function BuilderChat({ messages, isGenerating, onSendMessage, onRetry, currentCode = "", currentPlan = null, currentModel = "opencode/big-pickle", onModelChange, isBooting = false }: BuilderChatProps) {
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      >
        <div className="relative flex items-center gap-1.5 sm:gap-2">
          <Select value={currentModel} onValueChange={(v) => onModelChange?.(v)}>
            <SelectTrigger
              className="h-9 sm:h-10 shrink-0 w-[140px] sm:w-[170px] rounded-full bg-white/10 border border-white/20 backdrop-blur-md text-foreground text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-[#e052a0]/50"
