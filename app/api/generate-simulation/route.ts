/**
 * Simulation generation API. Generates interactive HTML5 Canvas/JS simulations.
...
const SIMULATION_RATE_LIMIT = { maxRequests: 5, windowMs: 60 * 1000 }

export async function POST(request: NextRequest) {
  try {
    const clientId = getClientIdentifier(request)
    const rateLimitResult = await checkRateLimitAsync(`simulation:${clientId}`, SIMULATION_RATE_LIMIT)
    if (!rateLimitResult.success) {
      return NextResponse.json(
        { error: 'Too many simulation generation requests. Please slow down.', retryAfter: rateLimitResult.retryAfter },
        { status: 429, headers: { 'Retry-After': String(rateLimitResult.retryAfter ?? 60), 'X-RateLimit-Remaining': '0' } }
      )
    }
...
The simulation should be educational and engaging.`
    ...
    let simulationCode = (rawCode || '').trim()
...
${simulationCode}
    ...
    if (user) {
      Promise.resolve(supabase.from('creations').insert({
        ...
        metadata: { originalPrompt: prompt, simulationType: type, complexity, status: 'completed' },
      })).catch((err: unknown) => {
    ...
    return NextResponse.json(
      { error: 'Failed to generate simulation', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
...
export async function GET() {
  return NextResponse.json({
    ...
    complexity: ['simple', 'medium', 'complex'],
    features: ['Interactive controls', 'Real-time physics', 'Canvas-based rendering', 'Educational content'],
  })
}
