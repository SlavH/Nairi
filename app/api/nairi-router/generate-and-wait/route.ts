/**
 * POST /api/nairi-router/generate-and-wait – generate then poll until result.
 * Body: { type, prompt, options? }. Returns final result or error.
 * Long-running: use for synchronous UX when client can wait.
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { job_id } = await routerGenerate(
      type as NairiRouterGenerationType,
      prompt,
      body.options
    )
    const result = await pollForResult(job_id, 2_500, 60)
    return NextResponse.json({ job_id, result })
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    return NextResponse.json(
      { error: "Generate and wait failed", details: msg },
      { status: 502 }
    )
  }
}
